...
export async function GET(request: NextRequest) {
  try {
    ...
    if (typeId) {
      lessonTypeConditions.push(eq(teoriLessonTypes.id, typeId));
    }
    ...
    if (typeId) {
      sessionConditions.push(eq(teoriSessions.lessonTypeId, typeId));
    }
    ...
    const sessionsByType = lessonTypes
      ...
      .filter((group: any) => group.sessions.length > 0);
