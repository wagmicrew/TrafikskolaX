...
const improvedTemplates = {
  ...
  new_password: {
    ...
    htmlContent: `
      ...
      <div style="background-color:#dcfce7; padding:16px; border-radius:8px; margin:16px 0; border-left:4px solid #16a34a;">
        <p style="margin:8px 0;"><strong>Ändrat:</strong> {{currentDate}} {{currentTime}}</p>
  ...
  try {
    ...
    for (const [triggerType, templateData] of Object.entries(improvedTemplates)) {
      ...
      if (existing) {
        ...
        const needsUpdate = !currentHtml.includes('data-standard-email="1"') ||
                           !currentHtml.includes('{{schoolPhone}}') ||
...
function getDefaultReceiversForTrigger(triggerType: string): string[] {
...
export { migrateEmailTemplatesDesign };
