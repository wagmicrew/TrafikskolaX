...
export default function SideditorClient() {
  ...
  const loadPageContent = useCallback(async (pageId: string) => {
    ...
    try {
      ...
      // Ask server to parse TSX into Puck Data using AST
      const parseRes = await fetch('/api/admin/sideditor/parse-tsx', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ content: fileContent })
      });
      let data: Data = { content: [], root: { props: {} } };
      if (parseRes.ok) {
        const parsed = await parseRes.json();
        data = (parsed?.data as Data) ?? data;
      }
