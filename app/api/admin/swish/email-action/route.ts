...
export async function POST(request: NextRequest) {
  try {
    ...
    if (sessionType === 'order') {
      ...
      if (action === 'confirm') {
        // Mark paid
        await db.update(packagePurchases).set({ paymentStatus: 'paid' as any, paidAt: new Date() }).where(eq(packagePurchases.id, p.id));
        ...
        for (const content of contents as any[]) {
          ...
          if (content.lessonTypeId) {
            ...
            if (existing.length) {
              ...
              await db.insert(userCredits).values({ userId: p.userId, lessonTypeId: content.lessonTypeId, creditsRemaining: addAmount, creditsTotal: addAmount, packageId: p.packageId, creditType: 'lesson' as any });
            }
            ...
            if (existing.length) {
              ...
              await db.insert(userCredits).values({ userId: p.userId, handledarSessionId: content.handledarSessionId, creditsRemaining: addAmount, creditsTotal: addAmount, packageId: p.packageId, creditType: 'handledar' as any });
            }
          }
        }
        ...
        try {
          ...
          const u = uRows[0] as any; const pkg = pkgRows[0] as any;
          const appUrl = process.env.NEXT_PUBLIC_APP_URL || '';
          await EmailService.sendTriggeredEmail('payment_confirmed', { user: { id: u.id, email: u.email, firstName: u.firstName, lastName: u.lastName, role: u.role }, booking: { id: p.id, lessonTypeName: `Paket: ${pkg?.name || ''}`, totalPrice: String(p.pricePaid || '0'), paymentMethod: 'swish' } } as any);
          await EmailService.sendTriggeredEmail('booking_confirmed', { user: { id: u.id, email: u.email, firstName: u.firstName, lastName: u.lastName, role: u.role }, booking: { id: p.id, lessonTypeName: 'Paket aktivt', totalPrice: String(p.pricePaid || '0') }, customData: { links: { bookingFlowUrl: `${appUrl}/boka-korning` } } } as any);
        } catch {}
      } else if (action === 'deny') {
        await db.update(packagePurchases).set({ paymentStatus: 'failed' as any }).where(eq(packagePurchases.id, p.id));
      } else if (action === 'remind') {
        // Send package payment reminder
        try {
          await EmailService.sendTriggeredEmail('package_payment_reminder', { user: { id: '', email: p.userEmail || '', firstName: '', lastName: '', role: 'student' }, customData: { links: { packagesPaymentUrl: `${process.env.NEXT_PUBLIC_APP_URL || ''}/cart?type=package&id=${p.id}` } } } as any);
        } catch {}
      }
      return NextResponse.json({ success: true });
    }
